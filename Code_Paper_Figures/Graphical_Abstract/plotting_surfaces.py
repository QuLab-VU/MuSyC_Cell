...
def plot_surface(fit_params, incl_gamma=False, data_pts=None, N=50, elev=20, azim=19, fname=None, zlim=None, zero_conc=1, alpha = 0.9,metric_name="DIP rate" + r'$h^{-1}$',path=None,title=None):
    ...
    my_cmap_rgb = plt.get_cmap('Purples')(np.arange(256))
...
#    cbar_ax = fig.add_axes([0.1, 0.9, 0.6, 0.05])
    ...
    surf = ax.plot_surface(tt, yy, zz, cstride=1, rstride=1, alpha=alpha, cmap = cm.Purples, vmin=zmin, vmax=zmax, linewidth=0)
    ...
    if incl_gamma:
        ...
        ax.scatter(d2_max*np.ones(y.shape), y, Edrug2D_NDB_hill((np.power(10.,y),np.power(10.,d2_max*np.ones(y.shape))),e0,e1,e2,e3,r1,r2,ec50_1,ec50_2,h1,h2,alpha1,alpha2,gamma1,gamma2), s=lw*10,c=rtop_map)
        ...
        ax.scatter(t, d1_max*np.ones(y.shape), Edrug2D_NDB_hill((np.power(10.,d1_max*np.ones(t.shape)),np.power(10.,t)),e0,e1,e2,e3,r1,r2,ec50_1,ec50_2,h1,h2,alpha1,alpha2,gamma1,gamma2), s=lw*10,c=btop_map)
        ...
        ax.scatter(d2_max*np.ones(y.shape), y, dip(y,d2_max, e0, e1, e2, e3, h1, h2, alpha1, alpha2, r1, r1r, r2, r2r), s=lw*2,c=rtop_map)
        ...
        ax.scatter(t, d1_max*np.ones(y.shape), dip(d1_max,t, e0, e1, e2, e3, h1, h2, alpha1, alpha2, r1, r1r, r2, r2r), s=lw*2,c=btop_map)
